...
import { LibrarItemDB, LibraryItem, LibraryItemOutput, S3Action, User } from '../types/user';
...
export class LibraryService {
  ...
  async dbInsertLibraryItem(user_id: number, item: LibrarItemDB, trx?: Knex.Transaction): Promise<LibrarItemDB> {
    try {
      const db = trx || this.db;
      const objects = await db('library_items as li')
        .insert({
  ...
  async ParseLibraryItemDbB(item: LibrarItemDB | LibraryItem, output: LibraryItemOutput): Promise<LibrarItemDB | LibraryItem> {
  ...
  async GetObject(user: User, path: string, trx?: Knex.Transaction): Promise<LibraryItem> {
    try {
      ...
      const libObj = await this.ParseLibraryItemDbB(itemDb, LibraryItemOutput.API) as LibraryItem;
      libObj.url = url;
      return libObj;
    } catch(err)  {
      console.log(err.message);
  ...
  async PutObject(user: User, params: LibraryItem, trx?: Knex.Transaction): Promise<LibraryItem> {
    try {
      ...
      const url = await this._storage.GetPresignedUrl(`${user.email}/${relativePath}`, S3Action.PUT);
      const libObj = await this.ParseLibraryItemDbB(params, LibraryItemOutput.DB) as LibrarItemDB;
      const itemDbInserted = await this.dbInsertLibraryItem(user.id_user, libObj);
      const apiResponse = await this.ParseLibraryItemDbB(itemDbInserted, LibraryItemOutput.API) as LibraryItem;
      apiResponse.url = url;
      return apiResponse;
    } catch(err)  {
      console.log(err.message);
